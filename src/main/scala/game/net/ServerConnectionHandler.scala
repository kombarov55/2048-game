...
import game.net.handlerbehavior.{IOBehavior, LobbyBehavior, RoomHostBehavior, SocketHandler}
...
class ServerConnectionHandler(val remoteAddress: InetSocketAddress, val localAddress: InetSocketAddress, val connection: ActorRef)
  extends Actor with SocketHandler with IOBehavior with LobbyBehavior with RoomHostBehavior {
